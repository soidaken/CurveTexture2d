...
CurveTexture2D 组件暴露了丰富的属性，方便使用者灵活配置地形的表现和行为。以下为主要属性说明：
...
| **同步模式 (syncOnOff, syncTarget)**     | 是否自动同步另一个节点的地形数据，实现多节点地形复用。                            |
| **物理属性相关**                         |                                                                                   |
...
- **PolygonCollider2D**（物理相关，后续章节详述）：用于同步生成与地形形状一致的碰撞体，实现物理交互。
